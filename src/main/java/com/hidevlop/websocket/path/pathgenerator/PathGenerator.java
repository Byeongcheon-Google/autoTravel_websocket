package com.hidevlop.websocket.path.pathgenerator;

import com.bcgg.path.pathgenerator.model.Spot;
import com.bcgg.path.util.DistanceCalculator;
import com.bcgg.path.util.MidpointCalculator;

import com.hidevlop.websocket.path.util.SpotUtil;
...
public class PathGenerator {
    ...
    public static Map<Spot.K, List<Spot>> generateGroup(@NotNull List<Spot> spotList) {
        ...
        Spot midSpot = new Spot.Normal(
                MidpointCalculator.calculate(spotList.stream().map(Spot::getLatitude).collect(Collectors.toList())),
                MidpointCalculator.calculate(spotList.stream().map(Spot::getLongitude).collect(Collectors.toList()))
        );
